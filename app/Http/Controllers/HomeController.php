...
class HomeController extends Controller
{
   ...
   protected function getNextKycStep(Userkyc $kyc)
{
    $steps = [
        1 => 'is_bank',
        2 => 'is_aadhar',
    ...
    foreach ($steps as $stepNumber => $field) {
        if ($kyc->$field != 1) { // 1 = verified
            return $stepNumber; // redirect to first incomplete step
        }
    }
    ...
    public function index(Request $request)
    {
       ...
       Example:
       https://nirk.dialfirst.in?referral_code=ABC123
    ========================================= */

    $referral_code = $request->referral_code;

    /* OPTIONAL:
       store in session so signup page can use it
    */
    if ($referral_code) {
        ...
        return view('frontend.' . get_setting('homepage_select') . '.index', compact('featured_categories','hot_categories', 'lang',  'referral_code'));
    }
    ...
    public function login()
{
    if (Auth::check()) {
        ...
        if ($user->user_type == 'customer') {
            ...
            // First time login: no steps completed at all
            if ($kyc->is_bank == 2 && $kyc->is_aadhar == 2 && $kyc->is_pan == 2) {
                return view('frontend.kyc.pending'); // show pending blade with "Complete KYC" button
            }

            // Partial KYC completed: redirect to next incomplete step
            $nextStep = $this->getNextKycStep($kyc);
            if ($nextStep > 0) {
                return redirect()->route('user.kyc', ['step' => $nextStep]);
            }
        }
    }

    // Seller / delivery boy login
    ...
    public function registration(Request $request)
{
    ...
    if ($request->has('referral_code')) {
        ...
        // SAVE COOKIE
        Cookie::queue('referral_code', $referral_code, 43200);

        // OPTIONAL AFFILIATE PROCESS
    ...
    return view(
        ...
        compact('email', 'phone', 'referral_code')
    );
}
    ...
    public function agentRegistration(Request $request)
{
    ...
    if ($request->has('referral_code')) {
        ...
        $referral_code = Cookie::get('agent_referral_code');
    }

    return view(
        'auth.'.get_setting('authentication_layout_select').'.agent_registration',
        compact('referral_code')
    );
}
...
return redirect()->route('agent.kyc', [
    ...
    return view('frontend.kyc.pending');
}

public function agentLogin()
{
    return view(
        'auth.'.get_setting('authentication_layout_select').'.agent_login'
    );
}
