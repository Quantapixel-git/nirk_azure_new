    ...
    public function storeReturn(Request $request)
{
    ...
    // ✅ Check delivered & paid condition
    if (
    ...
    if (!$order->delivered_date) {
        return response()->json([
            'status' => false,
            'message' => 'Delivered date not found'
        ]);
    }
    ...
    // ✅ Product return enabled check
    if (
        !$productReturn ||
        $productReturn->is_return != 1
    ) {
        return response()->json([
            'status' => false,
    ...
    // ✅ Calculate return validity from delivered_date
    $returnValidTill = Carbon::parse($order->delivered_date)
        ->addDays($productReturn->return_days);

    // ✅ Return expiry check
    if (Carbon::now()->gt($returnValidTill)) {
        return response()->json([
            'status' => false,
    ...
    $existing = OrderReturn::where('order_detail_id', $request->order_detail_id)
        ->first();
    ...
    if ($request->hasFile('image')) {
        ...
        $file->move(public_path('uploads/returns'), $image);
    }
