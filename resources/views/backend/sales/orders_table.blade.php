...
<div class="card-body">
        ...
        <thead>
            <tr>
                ...
                <th class="hide-xxl text-uppercase fs-12 fw-700 text-secondary">{{ translate('Payment Status') }}</th>
                 <th class="hide-xxl text-uppercase fs-12 fw-700 text-secondary">{{ translate('Return Status') }}</th>
    ...
    @if($returnRequest)
       ...
       <button
    type="button"
    id="return-badge-{{ $returnRequest->id }}"
    class="badge badge-inline {{ $badgeClass }} border-0 return-modal-btn"
    data-toggle="modal"
    data-target="#returnModal{{ $returnRequest->id }}">
    {{ translate($statusText) }}
</button>

        <!-- Modal -->
        <div class="modal fade" id="returnModal{{ $returnRequest->id }}" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">

                    <div class="modal-header">
                        <h5 class="modal-title">
                            {{ translate('Return Request Details') }}
                        </h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <strong>{{ translate('Product') }}:</strong><br>
                            {{ $product->name ?? 'N/A' }}
                        </div>
                        ...
                        <div class="mb-3">
                            <strong>{{ translate('Return Date') }}:</strong><br>
                            {{ \Carbon\Carbon::parse($returnRequest->created_at)->format('d M Y h:i A') }}
                        </div>
                        ...
                        @if($returnRequest->image)
                            <div class="mb-3">
                                ...
                                <img
                                    src="{{ asset('public/uploads/returns/'.$returnRequest->image) }}"
                   ...
                   @if(in_array($returnRequest->order_status, [2,3]))

                  <div class="modal-footer" id="return-footer-{{ $returnRequest->id }}">

   <form action="{{ url('/admin/return/update') }}" method="POST">
   ...
   <button
    ...
    onclick="testApprove({{ $returnRequest->id }})">
    Approve
</button>
...
function testReject(id)
{
    ...
    <form action="{{ route('admin.return.update') }}" method="POST" style="display:inline;">
        @csrf
        <input type="hidden" name="return_id" value="{{ $returnRequest->id }}">
    ...
    onclick="testReject({{ $returnRequest->id }})">
    Reject
</button>
    </form>
