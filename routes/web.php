...
Route::get('/agent/registration', 'agentRegistration')
    ->name('agent.registration')
    ->middleware('handle-demo-login');

Route::get('/agent/login', 'agentLogin')
    ->name('agent.login')
    ->middleware('handle-demo-login');
...
Route::post('/agent/login', 'agentLoginSubmit')
...
Route::group(['middleware' => ['customer', 'verified', 'unbanned']], function () {
    ...
    Route::post('/order-return/store', [PurchaseHistoryController::class, 'storeReturn'])
...
Route::controller(ContactController::class)->group(function () {
  ...
  Route::controller(MLMUserController::class)->group(function () {
    Route::get('/mlm/user', 'userwallet')->name('mlm.userwallet');
    });
    
    Route::post(
    '/mlm/withdraw/request',
    [MLMUserController::class,'withdrawRequest']
)->name('mlm.withdraw.request');


     Route::controller(KYCController::class)->group(function () {

    Route::get('/user/kyc/{step?}', [KYCController::class, 'index'])->name('user.kyc');
    Route::post('/save-bank-kyc', 'saveBank')->name('save.bank.kyc');

    Route::post('/save-aadhar-kyc', 'saveAadhar')->name('save.aadhar.kyc');

    Route::post('/save-pan-kyc', 'savePan')->name('save.pan.kyc');
});
Route::get('/agent/kyc/{step?}', [KYCController::class, 'agentIndex'])
    ->name('agent.kyc')
    ->middleware('auth');
...
Route::prefix('agent')
    ...
    ->group(function () {
        ...
        Route::get('/dashboard',
            [AgentController::class,'agentDashboard']
        )->name('agent.dashboard');

        Route::get('/users',
            [AgentController::class,'users']
        )->name('agent.users');

        Route::get('/earnings',
            [AgentController::class,'earnings']
        )->name('agent.earnings');
...
Route::post('/agent/profile/update', [AgentController::class, 'updateProfile'])
    ...
    Route::get('/users',
        ...
        Route::get('/users/create',
            [AgentController::class,'createUser']
        )->name('agent.users.create');

        Route::post('/users/store',
            [AgentController::class,'storeUser']
        )->name('agent.users.store');

        Route::get(
    '/users/{id}/orders',
    [AgentController::class,'userOrders']
)->name('agent.users.orders');

Route::get(
    '/orders/{id}',
    [AgentController::class, 'orderDetails']
)->name('agent.orders.details');


Route::get('/vendors', [AgentController::class,'vendors'])
    ->name('agent.vendors');

Route::get('/vendors/create', [AgentController::class,'createVendor'])
    ->name('agent.vendors.create');

Route::post('/vendors/store', [AgentController::class,'storeVendor'])
    ->name('agent.vendors.store');



});

Route::post('/store-mlm-wallet', function(Request $request){

    session([
        'mlm_wallet_used' => $request->wallet_used
    ]);

    return response()->json([
        'status' => true
    ]);
})->name('store.mlm.wallet');


Route::controller(MoreController::class)->group(function () {
    Route::get('website/contact', 'contact')->name('contact-us');
      Route::get('website/about', 'about')->name('about-us');
       Route::get('website/downloads', 'download')->name('downloads');
    
});

Route::post('website/contact/store', [MoreController::class, 'storeContact'])
    ->name('contact.store');
