...
use App\Http\Controllers\AgentTargetController;

use App\Http\Controllers\MLMWithdrawController;
...
Route::group(['prefix' => 'admin', 'middleware' => ['auth', 'admin', 'prevent-back-history']], function () {
    ...
    Route::post('/admin/return/update',
    [OrderController::class, 'updateReturnStatus']
    ...
    Route::controller(MLMController::class)->group(function () {
        Route::get('/mlm', 'index')->name('mlm.index');
        Route::post('/mlm/update', 'updatemlm')->name('mlm.update');

        Route::get('/mlm/edit/{level}', 'edit')->name('mlm.edit');
        Route::get('/mlm/user', 'user')->name('mlm.user');

        Route::post('/mlm/user/disable-wallet/{id}', [MLMController::class, 'disableWallet'])->name('mlm.disable.wallet');
Route::post('/mlm/user/reset-referrals/{id}', [MLMController::class, 'resetReferrals'])->name('mlm.reset.referrals');

Route::post(
    '/mlm/user/activate/{id}',
    [MLMController::class, 'activateMlmUser']
)->name('mlm.activate.user');

        //PTRange
        Route::get('/mlm/ptrange', [MLMController::class, 'ptrange'])->name('ptrange.index');
        Route::get('/mlm/ptrange/create', [MLMController::class, 'createPtrange'])->name('ptrange.create');

        Route::post('/mlm/ptrange/store', [MLMController::class, 'storePtrange'])->name('ptrange.store');
        Route::get('/mlm/ptrange/edit/{id}', [MLMController::class, 'editPtrange'])->name('ptrange.edit');
        Route::post('/mlm/ptrange/update/{id}', [MLMController::class, 'updatePtrange'])->name('ptrange.update');


        //PTValue
        Route::get('/mlm/ptvalue', [MLMController::class, 'ptvalue'])->name('ptvalue.index');

        Route::get('/mlm/ptvalue/create', [MLMController::class, 'createPtvalue'])->name('ptvalue.create');

        Route::post('/mlm/ptvalue/store', [MLMController::class, 'storePtvalue'])->name('ptvalue.store');

        Route::get('/mlm/ptvalue/edit/{id}', [MLMController::class, 'editPtvalue'])->name('ptvalue.edit');

        Route::post('/mlm/ptvalue/update/{id}', [MLMController::class, 'updatePtvalue'])->name('ptvalue.update');


        Route::get('/mlm-wallet-history', [MLMController::class, 'mlmWalletHistory'])
            ->name('mlm.wallet.history');

        Route::get(
            '/mlm-wallet-transactions/{user_id}',
            [MLMController::class, 'walletTransactions']
        )
            ->name('mlm.wallet.transactions');
    });
});


 Route::post('/admin/return/update',
    [MLMController::class, 'updateReturnStatus']
)->name('admin.return.update');
...
Route::post('/agent-targets/store', [AgentTargetController::class,'store'])
    ->name('agent.targets.store');
    
    
    Route::controller(MLMWithdrawController::class)->group(function () {

    Route::get(
        '/mlm/withdrawals',
        'index'
    )->name('mlm.withdrawals');

    Route::post(
        '/mlm/withdrawals/{id}/approve',
        'approve'
    )->name('mlm.withdraw.approve');

    Route::post(
        '/mlm/withdrawals/{id}/reject',
        'reject'
    )->name('mlm.withdraw.reject');
});
