...
class ShiprocketController extends Controller
{
    ...
    public function deliveryStatus()
    {
        try {
            ...
            $credential = ShiprocketCredential::first();

            if (!$credential) {
                return response()->json([
                    'success' => false,
                    'error' => 'Shiprocket credentials not found.'
                ], 500);
            }
            ...
            $orders = Order::where('shipping_method', 'shiprocket')
                ->where('shiprocket_status', '!=', 'delivered')
            ...
            foreach ($orders as $order) {
                ...
                $data = $statusResp->json('tracking_data');
                $statusCode = $data['shipment_status'] ?? null;
                $currentStatus = null;
                if (!empty($data['shipment_track']) && isset($data['shipment_track'][0]['current_status'])) {
                ...
                $map = [
                    'confirmed' => [1, 2, 3, 4, 5, 11, 27, 52],
                    'picked_up' => [19, 42, 15, 20, 13],
                    'on_the_way' => [6, 17, 18, 21, 22, 38, 39, 48, 49, 50, 51, 54, 55, 56, 57, 59, 68, 71, 72],
                    'delivered' => [7, 23, 26, 43],
                    'cancelled' => [8, 16, 9, 14, 10, 40, 41, 46, 45, 12, 24, 25, 44, 47, 75, 76, 77, 78],
                ];

                foreach ($map as $parent => $codes) {
                    if (in_array($statusCode, $codes, true)) {
                        $order->delivery_status = $parent;
                        $order->orderDetails()->update([
                            'delivery_status' => $parent
                        ]);
                        break;
                    }
                }

                if (in_array($statusCode, [7, 23], true)) {
                    if ($order->payment_status != 'paid') {
                        $order->payment_status = 'paid';
                    }
                }
                
                $statusLower = $currentStatus ? strtolower($currentStatus) : null;
                $order->shiprocket_status_code = $statusCode;
    ...
    public function orderStatus()
    {
        try {
            ...
            foreach ($orders as $order) {
                ...
                if ($order->shiprocket_order_status !== $statusLower) {
                    $order->shiprocket_order_status = $statusLower;
                    $order->save();
                }
            }
