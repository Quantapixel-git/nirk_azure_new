...
<?php $__env->startSection('content'); ?>
    ...
    <div class="aiz-titlebar text-left pb-5px">
        <div class="row align-items-center">
            <div class="col-auto">
                <?php if(isset($back_to) && $back_to== 'brands'): ?>
                <a class="fs-14 fw-400 d-inline-block" href="<?php echo e(route('brands.index')); ?>"> <i class="las la-angle-left"></i> <?php echo e(translate('Back to Brands')); ?></a>
                <?php elseif(isset($back_to) && $back_to== 'categories'): ?>
                <a class="fs-14 fw-400 d-inline-block" href="<?php echo e(route('categories.index')); ?>"><i class="las la-angle-left"></i> <?php echo e(translate('Back to Categories')); ?> </a>
                <?php else: ?>
                <h1 class="h3 fw-bold"><?php echo e(translate('All products')); ?></h1>
                <?php endif; ?>
            </div>
    ...
    <div class="card">
        ...
        <!--Nav Tab -->
         <div
                ...
                <ul class="nav nav-tabs border-0 " id="myTab" role="tablist">
                    <?php $__currentLoopData = $product_types; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product_type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link px-0 pb-15px fs-14 fw-500 <?php echo e($loop->first ? 'active' : ''); ?>"
                                data-toggle="tab" role="tab" aria-selected="<?php echo e($loop->first ? 'true' : 'false'); ?>"
                                id="<?php echo e(Str::slug($product_type)); ?>-tab"
                                onclick="changeTab(this, '<?php echo e(Str::slug($product_type)); ?>')" role="tab"
                                aria-controls="<?php echo e(Str::slug($product_type)); ?>">
            ...
            <div class="">
                <?php if($seller_type != 'seller' && auth()->user()->can('add_new_product')): ?>
                    <a href="<?php echo e(route('products.create')); ?>" class="position-relative overflow-hidden add-new-btn">
                        <span class="position-relative z-2 pr-15px fs-14 fw-500 text-blue label-text"><?php echo e(translate('Add New Product')); ?></span>
                ...
                <div class="card-header row  border-0 pb-0 mt-2">
                    ...
                    <div class="dropdown mb-2 mb-md-0 bg-light mt-2 mt-md-0 px-md-1 rounded-1">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            ...
                            <a class="dropdown-item confirm-alert text-secondary fs-14 fw-500 hov-bg-light hov-text-blue"
                                ...
                                <?php echo e(translate('Publish')); ?></a>
                            <a class="dropdown-item text-secondary fs-14 fw-500 hov-bg-light hov-text-blue" id="bulk-featured-option" onclick="bulkFeatured()"
                                ...
                                <?php echo e(translate('Mark Featured')); ?></a>
                            <a class="dropdown-item text-secondary fs-14 fw-500 hov-bg-light hov-text-blue" id="bulk-td-option" onclick="bulkProductTodaysDeal()"
                                href="javascript:void(0)">
                                <?php echo e(translate('Mark Todays Deal')); ?></a>
                            <?php endif; ?>
                    ...
                    <div class="col-md-2 mr-0 px-0 inner-select ml-1">
                        <select class="form-control  aiz-selectpicker mb-2 mb-md-0 bg-light" id="user_id" name="user_id" onchange="sort_products()">
                            <option value="" class="hov-bg-light text-secondary fs-14 fw-40"><?php echo e(translate('All Sellers')); ?></option>
                            <?php $__currentLoopData = App\Models\User::where('user_type', '=', 'seller')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $seller): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option class="hov-bg-light text-secondary fs-14 fw-40" value="<?php echo e($seller->id); ?>">
...
<?php $__env->startSection('script'); ?>
    <script type="text/javascript">
        ...
        let currentTab = '<?php echo e(Str::slug($product_types[0] ?? '')); ?>';
        let searchTimer;
        let seller_type = '<?php echo e($seller_type); ?>';
        let selected_filter = [];
        let brand_id = '<?php echo e($brand_id ?? ''); ?>';
        ...
        function update_published(el){
            ...
            $.post('<?php echo e(route('products.published')); ?>', {_token:'<?php echo e(csrf_token()); ?>', id:el.value, status:status}, function(data){
                if(data == 1){
                    ...
                    AIZ.plugins.notify('danger', '<?php echo e(translate('GST verification is pending for this account.')); ?>');
                }
                else if(data == 4){
                    AIZ.plugins.notify('warning', '<?php echo e(translate('Please assign GST details')); ?>');
                }
                else{
                    AIZ.plugins.notify('danger', '<?php echo e(translate('Something went wrong')); ?>');
                }
        ...
        function single_delete(productId) {
            $.ajax({
                url: "<?php echo e(route('products.destroy', ':id')); ?>".replace(':id', productId),
                type: 'GET',
                success: function(response) {
                    if (response == 1) {
                        AIZ.plugins.notify('success', '<?php echo e(translate('Selected item deleted successfully')); ?>');
                        hideBulkActionModal();
        ...
        function bulk_delete() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response == 1) {
                        AIZ.plugins.notify('success', '<?php echo e(translate('Selected items deleted successfully')); ?>');
                        hideBulkActionModal(); 
                        getProducts(currentTab);
                    }
                }
        ...
        function bulk_publish() {
            ...
            $.ajax({
                ...
                url: "<?php echo e(route('bulk-product-publish')); ?>",
                type: 'POST',
                data: data,
                cache: false,
                contentType: false,
                ...
                success: function (response) {
                    if(response == 1) {
                        AIZ.plugins.notify('success', '<?php echo e(translate('Selected items Published successfully')); ?>');
                        hideBulkActionModal(); 
                        getProducts(currentTab);
                    }
                },
        ...
        function bulk_feature() {
            ...
            $.ajax({
                ...
                url: "<?php echo e(route('bulk-product-featured')); ?>",
                type: 'POST',
                data: data,
                cache: false,
                contentType: false,
                ...
                success: function (response) {
                    if(response == 1) {
                        AIZ.plugins.notify('success','<?php echo e(translate('Selected items added in featured successfully')); ?>' );
                        hideBulkActionModal(); 
                        getProducts(currentTab);
                    }
                },
        ...
        function bulk_todays_deal() {
            ...
            $.ajax({
                ...
                url: "<?php echo e(route('bulk-product-todays-deal')); ?>",
                type: 'POST',
                data: data,
                cache: false,
                contentType: false,
                ...
                success: function (response) {
                    if(response == 1) {
                        AIZ.plugins.notify('success', '<?php echo e(translate('Selected products have been marked as Today’s Deals.')); ?>');
                        hideBulkActionModal();
                        getProducts(currentTab);
                    }
                }
        ...
        function singleDelete(productId) {
            showBulkActionModal();
            $('#confirmation-title').text('<?php echo e(translate('Delete Confirmation')); ?>');
            $('#confirmation-question').text('<?php echo e(translate('Are you sure you want to delete the selected product?')); ?>');
            $('#impact-message').text('<?php echo e(translate('This action cannot be undone. Once deleted, the product will be permanently removed.')); ?>');
        ...
        function bulkDelete() {
            if ($('.check-one:checked').length == 0) {
                AIZ.plugins.notify('danger', '<?php echo e(translate('Please select at least one item')); ?>');
                return;
            }
            showBulkActionModal();
            $('#confirmation-title').text('<?php echo e(translate('Delete Confirmation')); ?>');
            $('#confirmation-question').text('<?php echo e(translate('Are you sure you want to delete the selected products?')); ?>');
            $('#impact-message').text('<?php echo e(translate('This action cannot be undone. Once deleted, the products will be permanently removed.')); ?>');
        ...
        function bulkPublish() {
            ...
            showBulkActionModal();
            $('#confirmation-title').text('<?php echo e(translate('Publish Confirmation')); ?>');
            $('#confirmation-question').text('<?php echo e(translate('Are you sure you want to publish the selected products?')); ?>');
            $('#impact-message').text('<?php echo e(translate('Products already published will be skipped.')); ?>');
        ...
        function bulkProductTodaysDeal() {
            ...
            showBulkActionModal();
            $('#confirmation-title').text('<?php echo e(translate('Today’s Deal Confirmation')); ?>');
            $('#confirmation-question').text('<?php echo e(translate('Are you sure you want to make the selected products as Today’s Deals?')); ?>');
            $('#impact-message').text('<?php echo e(translate('Products already marked as Today’s Deals will be skipped.')); ?>');
        ...
        function bulkFeatured() {
            ...
            showBulkActionModal();
            $('#confirmation-title').text('<?php echo e(translate('Feature Confirmation')); ?>');
            $('#confirmation-question').text('<?php echo e(translate('Are you sure you want to mark the selected products as featured ?')); ?>');
            $('#impact-message').text('<?php echo e(translate('Products already marked as featured will be skipped.')); ?>');
        ...
        function getProducts(slug, page = 1) {
            ...
            $.ajax({
                url: `<?php echo e(route('products.filter' )); ?>?page=${page}`,
                method: 'GET',
                ...
                error: function() {
                    $('#tab-content').html(`
                        <div class="text-center py-2 my-4 w-100">
                            <h5 class="fs-16 fw-bold text-gray"><?php echo e(translate('Something Went Wrong')); ?></h5>
        ...
        function changeTab(button, statusSlug) {
            ...
            if(statusSlug =='inhouse-products'){
                ...
                seller_type = '<?php echo e($seller_type); ?>';
            }
        ...
        // Right Offcanvas JS Start
            ...
            function openRightcanvas(id, name) {
                ...
                $.ajax({
                    type: "GET",
                    url: "<?php echo e(route('stock.show', '')); ?>/" + id,
                    success: function (data) {
                        rightOffcanvas.innerHTML = data;
                    },
                    error: function () {
                        rightOffcanvas.innerHTML = '<p class="text-danger"><?php echo e(translate("Failed to load stock data")); ?></p>';
                    }
                });
            }
            // Close function
        ...
        function updateStocks(productId) {
            ...
            $.ajax({
                url: "<?php echo e(route('bulk-product-stock-update')); ?>",
                type: "POST",
                data: {
                    _token: '<?php echo e(csrf_token()); ?>',
                    stocks: formData,
                ...
                success: function(response) {
                    if(response == 1) {
                        AIZ.plugins.notify('success', '<?php echo e(translate('Stock updated successfully')); ?>');
                        closeRightcanvas();
                    }
                },
                error: function() {
...
<?php echo $__env->make('backend.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u635947223/domains/dialfirst.in/public_html/nirk/resources/views/backend/product/products/index.blade.php ENDPATH**/ ?>