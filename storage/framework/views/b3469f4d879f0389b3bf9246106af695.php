<script type="text/javascript">
    ...
    function submitShippingInfoForm(el) {
        ...
        $.ajax({
            ...
            url: "<?php echo e(route('guest_customer_info_check')); ?>",
            type: 'POST',
            data: {
                email : email,
                phone : phone
            ...
            success: function (response) {
                if(response ==  1){
                    $('#login_modal').modal();
                    AIZ.plugins.notify('warning', '<?php echo e(translate('You already have an account with this information. Please Login first.')); ?>');
                }
                else{
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                <?php if(get_active_countries()->count() == 1): ?>
                    if (response.data.address_data.country_id != <?php echo e(get_active_countries()->first()->id); ?>) {
                        get_states(<?php echo e(get_active_countries()->first()->id); ?>);
                    }
                <?php endif; ?>
            }
        });
    ...
    function edit_billing_address(address) {
        var url = '<?php echo e(route("billing_addresses.edit", ":id")); ?>';
        url = url.replace(':id', address);
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(obj != ''&& $('<select></select>').html(obj).find('option').length > 1) {
                    ...
                    $('[name="city_id"]').html('<option value=""><?php echo e(translate('No cities are available under this state.')); ?></option>');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(obj != ''&& $('<select></select>').html(obj).find('option').length > 1) {
                    ...
                    $('[name="cbilling_ity_id"]').html('<option value=""><?php echo e(translate('No cities are available under this state.')); ?></option>');
    ...
    function get_city_by_country(country_id){
        ...
        $.ajax({
            ...
            url: "<?php echo e(route('get-city-by-country')); ?>",
            type: 'POST',
            data: {
                country_id: country_id
            },
            success: function (response) {
                ...
                if(obj != '' && $('<select></select>').html(obj).find('option').length > 1) {
                    ...
                    $('[name="city_id"]').html('<option value=""><?php echo e(translate('No cities are available under this country.')); ?></option>');
                    $('[name="city_id"]').attr('disabled', true);
                    AIZ.plugins.bootstrapSelect('refresh');
                }
            }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if(obj != '' && $('<select></select>').html(obj).find('option').length > 1) {
                    ...
                    $('[name="billing_city_id"]').html('<option value=""><?php echo e(translate('No cities are available under this country.')); ?></option>');
                    $('[name="billing_city_id"]').attr('disabled', true);
                    AIZ.plugins.bootstrapSelect('refresh');
                }
            }
...
<?php /**PATH /home/u635947223/domains/dialfirst.in/public_html/nirk/resources/views/frontend/partials/address/address_js.blade.php ENDPATH**/ ?>